...
PROGMEM prog_uint16_t fadeArray[] = {
...
int lightArray[amountOfLights];  // lightArray is in bytes just values in range of 0 - 255
...
{   Serial.begin(57600);
    ...
    for(int led=0; led<amountOfLights; led++){
...
{   
    ...
    { 
      ...
      //int val = inoise(x, 0, perlinTimePosition); 
      int val = renderNoise(x, 0, perlinTimePosition); 
    ...
    for(int i=0;i<amountOfLights;i++)
    { 
      int difference = lightArray[i]; 
      analogWrite(i+2, difference);
      if(difference>127) Serial.print("*");
      else               Serial.print("-");
    }
